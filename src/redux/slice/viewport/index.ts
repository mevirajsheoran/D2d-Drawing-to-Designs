...
const viewportSlice = createSlice({
  ...
  reducers: {
    ...
    zoomIn(state) {
      const centerX = state.canvasWidth / 2 || 400;
      const centerY = state.canvasHeight / 2 || 300;
      const next = clamp(state.scale * 1.2, state.minScale, state.maxScale);
      const t = zoomAroundScreenPoint(
        { x: centerX, y: centerY },
        next,
        state.translate,
    ...
    zoomOut(state) {
      const centerX = state.canvasWidth / 2 || 400;
      const centerY = state.canvasHeight / 2 || 300;
      const next = clamp(state.scale * 0.8, state.minScale, state.maxScale);
      const t = zoomAroundScreenPoint(
        { x: centerX, y: centerY },
        next,
        state.translate,
